...
class traj_follow_ctrl:
    def __init__(self, client) -> None:
        ...
        # Control Parameters
        self.kp = [1, 1 ,1, 2]# in order x, y, z, yaw
        self.ki =  [0, 0, 0, 0]
        self.kd = [0, 0, 0, 0]
    ...
    def task_cross_circle(self, circle_id):
        self.reset()
        while True:
            # waypoint_p = np.array([0,0,-20])
            # waypoint_v = np.array([0,0,0])
            # self.traj_pid_update(waypoint_p,waypoint_v,1.57)
            self.moveByMinSnapAsync(circle_id)
            state = self.client.getMultirotorState()
            position = state.kinematics_estimated.position
...
if __name__ == "__main__":
    client = traj_follow_ctrl('127.0.0.1')
    client.begin_task()
    #v_cmd = traj_follow_ctrl.pid()
